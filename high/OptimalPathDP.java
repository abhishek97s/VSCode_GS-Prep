...
public class OptimalPathDP {
    ...
    public static void optimalPathFinder(Integer[][] grid, int row, int col, int[][] gridValue) {
        ...
        System.out.print(gridValue[row][col]);
        return;
    }
    ...
    public static Integer optimalPath(Integer[][] grid) {
        ...
        if (grid.length == 0) 
            ...
            final int row = grid.length;  //rows
            final int col = grid[0].length;   // cols
        
            for (int i = row-1; i >= 0; i--) //run through rows
              for (int j = 0; j <col; j++) // run through colums
                if(i==row-1 && j>0)
                  grid[i][j] += grid[i][j-1];
                else if(i< row-1 && j==00)
                    grid[i][j]+= grid[i+1][j];
